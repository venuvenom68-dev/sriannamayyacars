...
// Website URL: derive from the current address (admin :4000 → site :3000) so
// it works on localhost AND on a phone via the LAN IP; fall back to env.
const siteUrl = () => {
  if (typeof window !== "undefined") return window.location.origin.replace(":4000", ":3000");
  ...
  return (
    <div className="admin-login">
      <form className="login-card" onSubmit={submit}>
        ...
        <a href={siteUrl()} target="_blank" rel="noopener" style={{ display: "inline-block", marginTop: 18, color: "var(--mute)", fontSize: 12 }}>Open website ↗</a>
      </form>
    </div>
  );
}
      ...
      <header className="admin-hd">
        <div className="wrap">
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 10 }}>
            <a className="btn btn-line" href={siteUrl()} target="_blank" rel="noopener">View website</a>
